...
MCP server is a thin HTTP client). The mirror exists so MCP tool
responses can render display labels without leaking substrate IDs
per ADR 2026-04-25.

Keep this map in lockstep with:
  - src/content_checker/labels.py:DISPLAY_LABELS (engine — source of truth)
  - cli-client/contentrx/display_labels.py
