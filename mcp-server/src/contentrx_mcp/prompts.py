...
def build_review_ui_copy_prompt(focus: str | None = None) -> str:
    ...
    return (
        ...
        + (
            ...
            "3. For any violation cited, call `explain_violation(standard_id)` "
            "if you need more rationale or examples to write a good "
            "suggested fix.\n"
            "\n"
            ...
            "each violation, cite the standard ID, quote the offending "
            "string, and propose a rewrite._\n"
            "\n"
            "## Standards cited\n"
            "_List every standard ID that fired, with a one-line "
            "summary of what it enforces. Use `list_standards()` if "
            "you need the rule text._\n"
            "\n"
            "Stay specific. No generic advice. Every claim ties back "
            "to a standard ID a maintainer can look up."
        )
    )
