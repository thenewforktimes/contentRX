...
DPA. Generated by a Python script using reportlab; the script is
not committed to the repo (it lives in the session outputs only —
regenerate by re-running the same generator if entity details,
subprocessor list, or technical measures change). Header carries
a prominent "starter template, requires legal review for material
reliance" warning box. SCCs (Exhibit D) auto-incorporate when
EU/UK access opens. Referenced from `/terms` §8 as a downloadable
PDF.
