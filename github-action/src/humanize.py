...
humanize.py`, `lsp-server/src/contentrx_lsp/humanize.py`, and
`mcp-server/src/contentrx_mcp/humanize.py`. Each surface keeps its
own copy because the GitHub Action is a thin wrapper around the
CLI and doesn't import from the engine. The vocabulary is locked
by ADR 2026-04-29 §9 — keep all five surfaces in sync when that
vocabulary updates.
